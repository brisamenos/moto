...
// GET /api/caixa/lancamentos?loja_token=xxx&data=2024-01-15
router.get('/lancamentos', (req, res) => {
  const { loja_token, data } = req.query;
...
router.post('/lancamentos', (req, res) => {
  ...
  db.prepare(`INSERT INTO caixa_lancamentos (id, tipo, descricao, forma_pagamento, valor, data, hora, loja_token)
              VALUES (?, ?, ?, ?, ?, ?, ?, ?)`
  ).run(id, l.tipo, l.descricao || '', l.forma_pagamento || 'dinheiro', Number(l.valor),
        data, hora, l.loja_token || 'padrao');
...
// GET /api/caixa/sessoes?loja_token=xxx&limit=1
...
router.put('/sessoes/:id', (req, res) => {
  ...
  db.prepare(`UPDATE caixa_sessoes SET status=?, valor_abertura=?, abertura_at=?, fechamento_at=?, updated_at=? WHERE id=?`)
    .run(s.status, Number(s.valor_abertura) || 0, s.abertura_at || null, s.fechamento_at || null, now, id);
